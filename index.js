...
app.post("/sendmail3", async (req, res) => {
  ...
  try {
    let transporter = nodemailer.createTransport({
      host: "mail.proactsys.com.my",
      port: 465,
      auth: {
        user: "ares@proactsys.com.my",
        pass: "Proactsys@1",
      },
    });
    let info = await transporter.sendMail({
...
app.post("/sendmail2", async (req, res) => {
  ...
  try {
    let transporter = nodemailer.createTransport({
      host: "promesasdelchocolate.com",
      port: 465,
      auth: {
        user: "manager@promesasdelchocolate.com",
        pass: "g8VwjS3M##824",
      },
    });
    let info = await transporter.sendMail({
      from: '"Happy time" <manager@promesasdelchocolate.com>',
